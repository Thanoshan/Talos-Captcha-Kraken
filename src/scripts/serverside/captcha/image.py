...
DEFAULT_FONTS = [os.path.join(DATA_DIR, "DroidSansMono.ttf"), os.path.join(DATA_DIR, "PlayfairDisplay-Black.ttf"), os.path.join(DATA_DIR, "COMIC.TTF")]#, "Smokum-Regular.ttf", "PlayfairDisplay-Black.ttf"
...
class WaveDeformer:
    ...
    def transform(self, x, y):
        y = y + 5*math.sin(x/(40 - 35*self.u) + (self.p *self.u * 1000)) * self.p
        return x, y
...
class WaveDeformer2:
    ...
    def transform(self, x, y):
        x = x + 0.5*math.sin(x/(20 - 18*self.u) + (self.p *self.u * 1000)) * self.p
        return x, y
...
class ImageCaptcha(_Captcha):
    ...
    def create_noise_dots(image, color, width=3, number=30, scale=1):
        ...
        while number:
            ...
            draw.line(((x1, y1), (x1 + random.randint(-2*scale, 2*scale), y1 + random.randint(-2*scale, 2*scale))), fill=color, width=random.randint(1, 3)*scale)
            number -= 1
        return image
    ...
    def generate_image(self, chars):
        ...
        self.create_noise_dots(im, color, number=random.randint(0, 30), scale=self.scale)
        while(random.uniform(0, 1) < 0.5):
            self.create_noise_curve(im, color)
        im = im.filter(ImageFilter.SMOOTH)
        im = self.do_random_filter(im)
        self.create_noise_dots(im, color, number=random.randint(0, 10), scale=self.scale)
        return im
